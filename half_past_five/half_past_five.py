...
class url(object):
    ...
    def __call__(self, cls):
        server.add_handlers(
            r".*$",
            (hpf_url(self.url, cls, name=self.name or (cls.__name__ + self.suffix)),),
        )
        return cls
...
class MainHandler(BaseHandler):
    ...
    def post(self):
        ...
        out = subprocess.run(
            ["yt-dlp", url, "-f", "140", "-o", download_path, "--print-json"],
...
class CropHandler(BaseHandler):
    ...
    def post(self):
        ...
        timedelta_end = timedelta(minutes=end[0], seconds=end[1], milliseconds=end[2])
        ...
        ffmpeg_delay = "%0.2d:%0.2d.%0.3d" % (
            int(timedelta_delay.seconds / 60),
            timedelta_delay.seconds % 60,
        ...
        self.set_header("Content-Disposition", f"attachment; filename={user_filename}")
        self.write(song)
